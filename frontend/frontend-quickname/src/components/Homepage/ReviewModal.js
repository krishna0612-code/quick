...
const ReviewModal = ({ onClose, onReviewSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/api/submit-review/", {
        method: "POST",
        headers: {
      ...
      // Create frontend review object for immediate UI update
      const newReview = {
        id: Date.now(),
        name: review.name.trim(),
        email: review.email.trim(),
        ...
        date: new Date().toISOString().split('T')[0],
        status: 'approved',
        avatar: review.name.trim().split(' ').map(n => n[0]).join('').toUpperCase()
      };
      ...
      console.error('Error submitting review:', error);
      alert(`There was an error submitting your review: ${error.message}. Please try again.`);
    } finally {
      setIsSubmitting(false);
    }
  };
